...
class BM25Index:
    def __init__(self, cache_path: str = "data/bm25_corpus.json"):
        ...
        else: print(f"[Warning] BM25 cache not found at {cache_path}\n   Run: python build_bm25_cache.py")

    def _load(self, path):
        try:
            from rank_bm25 import BM25Okapi
        except ImportError:
            print("[Warning] rank_bm25 not installed — pip install rank_bm25"); return
        try:
            ...
            self.ready = True
            print(f"[OK] BM25 index: {len(self.documents)} documents")
        except Exception as e: print(f"[Warning] BM25 load error: {e}")
...
class SpellCorrector:
    ...
    def correct(self, query: str) -> str:
        ...
        result = ' '.join(fixed)
        if changed: print(f"[Tool] Spell corrected: '{query}' → '{result}'")
        return result
...
class EnhancedHybridRetriever:
    ...
    def __init__(self, pinecone_index: str = "pdf-knowledge-base"):
        ...
        try:
            ...
            print(f"[Warning]  Neo4j unavailable (skipping graph context): {e}")
            self.neo4j_client = None
        self.bm25 = BM25Index()
        self.reformulator = LLMReformulator()
        self.spell_corrector = SpellCorrector()
        self.streaming_llm = StreamingLLM()  # [OK] Gemini only
    ...
    def retrieve(self, query: str, top_k: int = 6) -> RetrievedContext:
        ...
        if corrected != query.lower().strip(): print(f"[Tool] Corrected: {corrected}")
        print(f"[Search] Search queries ({len(all_queries)}): {all_queries}")
        ...
        print(f"\n[Metrics] Retrieved {len(final_results)} unique chunks:")
    ...
    # ─────────────────────────────────────────────────────────────
    # [OK] Streaming Method — Gemini Only
    # ─────────────────────────────────────────────────────────────
    ...
    def _get_graph_context(self, results):
        ...
        try:
            ...
            if items: print(f"[Graph] Retrieved {len(items)} graph nodes")
            return ctx
        except Exception as e: print(f"[Warning] Neo4j: {e}"); return {'context': []}
