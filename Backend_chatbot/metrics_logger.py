...
# Redis (Upstash) storage for metrics — survives every redeploy of this
# ephemeral container, unlike the local JSONL file below. Records are kept
# in a sorted set keyed by write time so we can range-query by window
# (ZRANGEBYSCORE) and trim anything past the retention window (ZREMRANGEBYSCORE)
# on every write. The dashboard's longest window is 30d, so nothing is ever
# lost that the dashboard could actually show; the extra days are just a
# buffer. This bounds storage permanently instead of growing forever —
# comfortably inside Upstash's free-tier limit at any realistic usage volume.
METRICS_ZSET_KEY = "metrics:events"
METRICS_RETENTION_SECONDS = 35 * 86400


def _window_start_ts(window: str, now_ts: float) -> float:
    if window == "24h":
        return now_ts - 24 * 3600
    elif window == "7d":
        return now_ts - 7 * 86400
    return now_ts - 30 * 86400  # 30d default
...
def log_request_metrics(
    ...
    """
    Record a single request. Thread-safe.

    Writes to Redis (Upstash) when it's actually connected (redis_client.use_local
    is False) so data survives redeploys. Falls back to the local JSONL file
    (this container's own ephemeral disk, wiped on every redeploy) only when
    Redis itself is unreachable, so metrics keep working either way.
    ...
    record = {
        ...
        # Sorted-set members must be unique, or two records logged in the
        # same instant collapse into one entry. This has no meaning beyond that.
        "_nonce": random.getrandbits(32),
    }
    payload = json.dumps(record)

    if not redis_client.use_local:
        try:
            client = redis_client.client
            with _log_lock:
                client.zadd(METRICS_ZSET_KEY, {payload: now_ts})
                client.zremrangebyscore(METRICS_ZSET_KEY, 0, now_ts - METRICS_RETENTION_SECONDS)
            return
        except Exception as e:
            print(f"⚠️ Redis metrics write failed, falling back to local file: {e}")

    # Local file fallback — only reached when Redis is unreachable.
    try:
...
def _summarize(records: list, user_id=None):
    """
    Aggregate an already time-window-filtered list of records into the
    dashboard summary shape. Shared by both the Redis and local-file paths
    so they stay identical regardless of where the records came from.
    """
    if user_id:
...
def get_metrics_summary(window="30d", user_id=None):
    """
    Compute the aggregated summary for the dashboard.

    Reads from Redis (Upstash) when it's connected — the durable,
    cross-redeploy store — and only falls back to the local JSONL file
    (this container's own ephemeral disk) when Redis is unreachable.
    ...
    if not redis_client.use_local:
        try:
            client = redis_client.client
            start_ts = _window_start_ts(window, now_ts)
            raw = client.zrangebyscore(METRICS_ZSET_KEY, start_ts, now_ts)
            records = []
            for item in raw:
                try:
                    ...
                    continue
            return _summarize(records, user_id)
        except Exception as e:
            print(f"⚠️ Redis metrics read failed, falling back to local file: {e}")
    ...
    return _summarize(filtered, user_id)
