...
# Global shared instances to prevent costly duplicate model/index/client initialization on per-session chatbot creations
_shared_pinecone_client = None
_shared_neo4j_client = None
_shared_bm25_index = None
_shared_spell_corrector = None
_shared_streaming_llm = None
_shared_neo4j_initialized = False

def get_shared_pinecone_client(index_name):
    global _shared_pinecone_client
    if _shared_pinecone_client is None:
        _shared_pinecone_client = PineconeClient(index_name)
    return _shared_pinecone_client

def get_shared_neo4j_client():
    global _shared_neo4j_client, _shared_neo4j_initialized
    if not _shared_neo4j_initialized:
        try:
            _shared_neo4j_client = Neo4jClient()
        except Exception as e:
            print(f"⚠️  Neo4j unavailable (skipping graph context): {e}")
            _shared_neo4j_client = None
        _shared_neo4j_initialized = True
    return _shared_neo4j_client

def get_shared_bm25_index():
    global _shared_bm25_index
    if _shared_bm25_index is None:
        _shared_bm25_index = BM25Index()
    return _shared_bm25_index

def get_shared_spell_corrector():
    global _shared_spell_corrector
    if _shared_spell_corrector is None:
        _shared_spell_corrector = SpellCorrector()
    return _shared_spell_corrector

def get_shared_streaming_llm():
    global _shared_streaming_llm
    if _shared_streaming_llm is None:
        _shared_streaming_llm = StreamingLLM()
    return _shared_streaming_llm


def reload_shared_indices():
    """Reload the in-memory BM25 index and spell corrector caches.
    This is called when new documents are uploaded and indexed.
    """
    global _shared_bm25_index, _shared_spell_corrector
    print("🔄 Hot-reloading BM25 index and spell corrector in-memory caches...")
    _shared_bm25_index = BM25Index()
    _shared_spell_corrector = SpellCorrector()
...
class EnhancedHybridRetriever:
    ...
    def __init__(self, pinecone_index: str = "pdf-knowledge-base"):
        self.pinecone_client = get_shared_pinecone_client(pinecone_index)
        self.neo4j_client = get_shared_neo4j_client()
        self.bm25 = get_shared_bm25_index()
        self.reformulator = LLMReformulator()
        self.spell_corrector = get_shared_spell_corrector()
        self.streaming_llm = get_shared_streaming_llm()
