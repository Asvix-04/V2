...
from utils import MAX_AUDIO_BYTES, s2s_limiter, RateLimiter
...
def _get_or_create_session(session_id: Optional[str]) -> tuple[str, PDFChatbot]:
    ...
    if not session_id or session_id not in _sessions:
        ...
        _sessions[session_id] = PDFChatbot()
...
def _run_pdf_pipeline(job_id: str, pdf_paths: List[str]) -> None:
    ...
    try:
        ...
        # ── Update combined book corpus ──
        _set("processing", "Updating combined book corpus…")
        from glob import glob
        txt_dir = os.path.join("data", "txts")
        all_txt_files = sorted(glob(os.path.join(txt_dir, "*.txt")))
        combined_texts = []
        for txt_path in all_txt_files:
            fname = os.path.basename(txt_path)
            if fname == "combined_book.txt":
                continue
            with open(txt_path, "r", encoding="utf-8") as f:
                content = f.read()
            combined_texts.append(f"\n\n{'='*70}\n=== SOURCE: {os.path.splitext(fname)[0]}.pdf ===\n{'='*70}\n\n{content}")

        combined_path = os.path.join(txt_dir, "combined_book.txt")
        with open(combined_path, "w", encoding="utf-8") as f:
            f.write("\n".join(combined_texts))

        # ── Rebuild BM25 search index and spell correction cache ──
        _set("processing", "Rebuilding BM25 index and spell corrector cache…")
        from build_bm25_cache import build_cache
        build_cache(txt_path=combined_path)

        # ── Hot-reload retrieval search indexes ──
        _set("processing", "Hot-reloading retrieval search indexes…")
        from hybrid_retriever import reload_shared_indices
        reload_shared_indices()

        _set("done", f"Successfully processed, indexed, and hot-reloaded {total} PDF(s).")
