...
# Detects a request to answer a NUMBERED question from an uploaded PDF, e.g.
# "answer question 28 from the pdf", "what is the answer to question 28", "q28".
PDF_QUESTION_REF_RE = re.compile(
    r'\b(?:question|ques|q)\.?\s*(?:no\.?|number|#)?\s*(\d{1,3})\b',
    re.IGNORECASE,
)
...
class PDFChatbot:
    ...
    def _resolve_pdf_question_reference(self, question: str):
        """Handle "answer question N from the pdf" requests.

        Looks up question N's ACTUAL text in the uploaded PDF(s) so the bot can
        answer that question from its knowledge base instead of trying to answer
        the literal request. Returns the question text if found, "" if a number
        was referenced but not found, or None if this isn't such a request."""
        if not self._uploaded_docs:
            return None
        m = PDF_QUESTION_REF_RE.search(question)
        if not m:
            return None
        n = int(m.group(1))
        from relevance_filter import extract_question_by_number
        for stem, text in self._uploaded_docs.items():
            found = extract_question_by_number(text, n)
            if found:
                print(f"📒 Resolved 'question {n}' from uploaded PDF '{stem}': {found!r}")
                return found
        print(f"📒 'question {n}' referenced but not found in any uploaded PDF")
        return ""
    ...
    def ask_question(self, question: str, use_history: bool = True, model: str = None) -> Dict[str, Any]:
        ...
        # PDF question lookup: "answer question 28 from the pdf" → fetch Q28's actual
        # text from the uploaded PDF and answer THAT from the knowledge base.
        resolved_q = self._resolve_pdf_question_reference(question)
        if resolved_q == "":
            msg = ("I couldn't find that question number in your uploaded PDF. "
                   "Please check the question number and try again.")
            self._record_turn(question, msg, use_history=use_history)
            return {'answer': msg, 'sources': [], 'vector_results': [],
                    'graph_context': {}, 'expanded_queries': [], 'validation': {}}
        if resolved_q:
            print(f"📒 Rewriting to PDF question text: {resolved_q!r}")
            question = resolved_q
    ...
    def ask_question_stream(
        ...
        # ── PDF question lookup: "answer question 28 from the pdf" → rewrite ──
        resolved_q = self._resolve_pdf_question_reference(question)
        if resolved_q == "":
            yield ("I couldn't find that question number in your uploaded PDF. "
                   "Please check the question number and try again.")
            return
        if resolved_q:
            question = resolved_q
...
SELF-CHECK: Before finishing, scan your answer. For every name, date, and specific fact — is it in the material above? If not, delete it."""
    ...
    def set_history(self, history: Optional[List[Dict[str, Any]]] = None):
        """Replace conversation history with a client-supplied list of turns.

        Used by the /history/sync endpoint so a returning user's prior turns
        seed the chatbot's context. Normalizes each turn and drops incomplete ones.
        """
        normalized_history = []
