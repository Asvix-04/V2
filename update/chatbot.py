...
class PDFChatbot:
    ...
    def _load_uploaded_docs(self) -> Dict[str, str]:
        ...
        try:
            ...
            print(f"[Warning] Could not load uploaded docs for trust check: {e}")
        if docs:
            print(f"[Attachment] Trusted uploaded docs: {list(docs.keys())}")
        return docs

    def _sync_session_history(self, session_id: str) -> None:
        """Syncs local conversation_history with the global Redis session."""
        global_history = redis_client.get_session_history(session_id)
        if global_history is not None:
            self.conversation_history = global_history
    ...
    def _query_matches_uploaded_doc(self, question: str) -> bool:
        ...
        if self._uploaded_docs:
            print("[Attachment] Uploaded doc present — trusting query past strict validation")
            return True
        return False
    ...
    def _classify_input(self, question: str) -> str:
        ...
        print(f"[Info] Rule-based classifier: '{q[:60]}' → {result} (greeting={has_greeting}, syllabus_kw={has_syllabus}, non_media={has_non_media})")
        return result
    ...
    def ask_question(self, question: str, use_history: bool = True, model: str = None, session_id: str = None) -> Dict[str, Any]:
        ...
        if exact_match:
            print("[OK] [Cache] Exact Match Hit!")
        ...
        # ── Global Cache Hook 2: Semantic Match (200ms timeout guard) ──
        # NOTE: Manual executor (not `with`) so shutdown(wait=False) truly
        # releases the thread on timeout instead of blocking until Pinecone finishes.
        semantic_hash = None
        _exec = concurrent.futures.ThreadPoolExecutor(max_workers=1)
        try:
            _future = _exec.submit(
                self.retriever.pinecone_client.search_semantic_cache,
                q_clean, 0.95
            )
            semantic_hash = _future.result(timeout=0.2)  # 200ms max
            ...
            _exec.shutdown(wait=False)  # don't block — let thread finish in background

        if semantic_hash:
            semantic_match = redis_client.get_by_hash(semantic_hash)
            if semantic_match:
                print("[OK] [Cache] Semantic Match Hit!")
        ...
        if self._contains_profanity(question):
            return {
                'answer': "[Warning] Please keep the conversation respectful.",
                'sources': [], 'vector_results': [],
                'graph_context': {}, 'expanded_queries': [], 'validation': {}
            }
        if self._contains_harmful_content(question):
            return {
                'answer': "[Warning] This type of question is not supported. Such topics may be illegal or harmful. Please ask questions related to Media Literacy.",
                'sources': [], 'vector_results': [],
                'graph_context': {}, 'expanded_queries': [], 'validation': {}
            }
        ...
        try:
            ...
            if not is_trusted_upload and classification == "out_of_syllabus" and not likely_followup and top_score < 0.040:
                print(f"[Speed] Fast-reject: classification is out_of_syllabus and top_score {top_score:.4f} < 0.040")
            ...
            if not is_trusted_upload and top_score < min_score_gate:
                print(f"[Warning] Top score {top_score:.4f} below gate {min_score_gate:.3f} — refusing")
                self._record_turn(question, OUT_OF_SCOPE_MESSAGE, use_history=use_history,
                                  sources=source_meta,
                                  expanded_queries=retrieved_context.expanded_queries,
                                  validation={})
            ...
            if top_score >= 0.06:
                print(f"[Metrics] High retrieval score ({top_score:.4f}) — validating topic relevance...")
            ...
            if is_high_conf or is_keyword_syllabus_hit or is_trusted_upload:
                ...
                print(f"[OK] High-confidence/keyword syllabus hit ({top_score:.4f}) — skipping validation ({reason})")
                validation_result = {
            ...
            if validation_result.get('_validation_error', False):
                print("[Warning] Validation error — using score-based fallback")
                ...
                print(f"[Metrics] Validation — score: {val_score}/10 | main_subject: {is_main}")
            ...
            print(f"[LLM] Generating answer (intent: {response_intent.followup_mode}/{response_intent.tone_signal})...")
            ...
            if not is_out_of_scope:
                ...
                # that aren't used in the final API response anyway
    ...
    def generate_follow_up_questions(
        ...
        try:
            ...
            print(f"[Warning] Error generating follow-up questions: {e}")
            return {
                "type_2_context_aware": [],
                "follow_up_items": [],
                "follow_up_markdown_links": [],
    ...
    def ask_question_stream(
        ...
        # ── Safety gates ──
        if self._contains_profanity(question):
            yield "[Warning] Please keep the conversation respectful."
            return
        if self._contains_harmful_content(question):
            yield "[Warning] This type of question is not supported."
            return
...
{{"completeness_score": <integer 1-10>, "can_fully_answer": <true or false>, "is_main_subject": <true or false>, "topic_directly_discussed": <true or false>, "reasoning": "<one sentence>"}}"""
        ...
        try:
            ...
            if result is None:
                print(f"[Warning] Validation parse failed: {validation_text[:200]}")
                return {"completeness_score": 2, "can_fully_answer": False, "is_main_subject": False,
...
5. EXAMPLE VS SUBJECT — CRITICAL RULE:
   ...
   [Error] DO NOT define or explain the topic using your world knowledge
   [Error] DO NOT cherry-pick course mentions to construct an answer about a different topic
   [OK] DO answer only if the material explicitly TEACHES that topic as its subject
...
[Error] NEVER add people, researchers, or scholars not named in the material
[Error] NEVER add dates, years, or statistics not present in the material
[Error] NEVER add book titles, publication names, or citations not in the material
[Error] NEVER introduce theories or frameworks not referenced in the material
[Error] NEVER invent examples — use only examples explicitly from the material
[Error] NEVER say "The materials do not elaborate..." or "The provided material..."
[Error] NEVER write meta-commentary about what the sources do or don't contain
[Error] NEVER pad an answer with external knowledge to fill length
[Error] NEVER use world knowledge to define a topic that only appears as a passing example
