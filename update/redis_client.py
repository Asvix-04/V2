...
class LocalMemoryCache:
    ...
    Mirrors Redis TTL behaviour: setex() stores an expiry timestamp,
    and get() silently evicts keys that have passed their TTL.
    """
    def __init__(self):
    ...
    def get(self, key: str) -> Optional[str]:
        ...
        if exp is not None and _time.time() > exp:   # TTL expired → evict
    ...
    def setex(self, key: str, time: int, value: str) -> None:
        ...
        self._expiry[key] = _time.time() + time       # store absolute expiry ✅

    def set(self, key: str, value: str) -> None:
        self._cache[key] = value
        self._expiry.pop(key, None)                   # no expiry for plain set()
    ...
    def __init__(self, host: str = "localhost", port: int = 6379, db: int = 0, default_ttl: int = 86400):
        ...
        if REDIS_AVAILABLE:
            try:
                ...
                print(f"[Redis] Redis unavailable at {redis_host}:{redis_port}, falling back to LocalMemoryCache")
                self.client = LocalMemoryCache()
        else:
            print("[Redis] Redis python package not found, falling back to LocalMemoryCache")
            self.client = LocalMemoryCache()
