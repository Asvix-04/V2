...
class PineconeClient:
    ...
    def upsert_chunks(self, chunks: List[Any], namespace: str = "", progress_callback=None) -> None:
        ...
        """
        if not chunks:
            print("[Warning]  upsert_chunks called with empty list — nothing to do.")
            return
        ...
        print(f"[Speed] Encoding {len(texts)} chunks in batches of {ENCODE_BATCH}...")
        ...
        print(f"\n[OK] Encoding complete — {len(all_embeddings)} embeddings ready")
        ...
        print(f"\n[OK] Upserted {upserted} vectors to Pinecone")
    ...
    def search(self, query: str, top_k: int = 5, namespaces: List[str] = ["", "uploads"]) -> List[Dict]:
        ...
        for ns in namespaces:
            try:
                ...
                print(f"[Warning]  [Search] Failed to query namespace '{ns}': {e}")
    ...
    def upsert_semantic_cache(self, question: str, redis_hash: str, namespace: str = "semantic_cache") -> None:
        ...
        print(f"[OK] [Cache] Upserted semantic cache vector for query: {question[:30]}...")
        ...
        try:
            ...
            if results.get('matches') and len(results['matches']) > 0:
                ...
                if best_match.get('score', 0.0) >= threshold:
                    print(f"[OK] [Cache] Semantic cache hit! Score: {best_match.get('score')} for: {query[:30]}...")
                    return best_match['metadata'].get('redis_hash')
        except Exception as e:
            print(f"[Warning]  [Cache] Semantic cache search failed: {e}")
