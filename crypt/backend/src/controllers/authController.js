    ...
    try {
        ...
        if (!auth) {
            return res.status(503).json({ 
                message: 'Google Authentication is not configured on the server. Please provide Firebase Admin credentials.' 
            });
        }
...
exports.sendOTP = async (req, res) => {
    try {
        ...
        if (!resend) {
            console.error('Cannot send OTP: Resend is not initialized (missing API key)');
